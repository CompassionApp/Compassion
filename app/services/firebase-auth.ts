...
import { User, GetUserResult } from "./api/api.types"
...
export class FirebaseAuth {
  ...
  async createUserWithEmailAndPassword(email: string, password: string): Promise<GetUserResult> {
    try {
      const userCredential = this.firebase.authentication.createUserWithEmailAndPassword(
        email,
        password,
      )
  ...
  async signInWithEmailAndPassword(email: string, password: string): Promise<GetUserResult> {
    try {
      const userCredential = this.firebase.authentication.signInWithEmailAndPassword(
        email,
        password,
      )
      const user = (await userCredential).user
